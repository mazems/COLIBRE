...
if(ngals > 0):
    ...
    gal_props = np.zeros(shape = (ngals, 14))
    gal_props[:,0] = sgn_in
    ...
    gal_props[:,10] = desc_id[select]
    gal_props[:,11] = track_id[select]
...
a = 2/3
logsigma = 8.0 
logsigma_ref = 9.9 #10.3 in Barro 2013
...
if ngals > 0:
    ...
    extras = (ucmgs > 0) & (ucmgs < 0.1)
    ucmg_ids_extras = sgn_in[extras]
    print(f"Number of additional UCMGs: {np.sum(ucmgs[extras])}")
    pd.DataFrame({"subhalo_id": ucmg_ids_extras}).to_csv("ucmg_ids_extras.csv", index=False)
