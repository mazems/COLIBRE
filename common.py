...
def read_group_data_eagle(model_dir, snap_file, fields, subvolumes):
    """Read the galaxies.hdf5 file for the given model/file_*subvolume"""

    data = collections.OrderedDict()
    for idx, subv in enumerate(subvolumes):

        fname = os.path.join(model_dir, 'groups_'+ snap_file, 'eagle_subfind_tab_'+ snap_file+ '.' + str(subv) + '.hdf5')
...
def read_particle_data_eagle(model_dir, snap_file, fields, subvolumes):
    """Read the galaxies.hdf5 file for the given model/file_*subvolume"""

    data = collections.OrderedDict()
    for idx, subv in enumerate(subvolumes):

        fname = os.path.join(model_dir, 'particledata_'+ snap_file, 'eagle_subfind_particles_'+ snap_file+ '.' + str(subv) + '.hdf5')
        logger.info('Reading particle data from %s', fname)
        with h5py.File(fname, 'r') as f:
            for gname, dsnames in fields.items():
                group = f[gname]
...
def read_particle_data_colibre(model_dir, snap_file, fields):
...
def read_particle_membership_colibre(model_dir, snap_file, fields):
    """Read the particles hdf5 file for the given model/file_*subvolume"""
