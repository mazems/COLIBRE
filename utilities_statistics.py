...
def wmedians_variable_bins(x=None, y=None, nbins = 5, add_last_bin = True):

    #define size of bins, assuming bins are all equally spaced.
    ngals = len(x)
    ngals_per_bin = int(np.floor((ngals + 0.0) / (nbins)))
    ...
    print("ngals_per_bin", ngals_per_bin)
    xmed = np.zeros(shape = nbins_total) 
    result = np.zeros(shape = (3, nbins_total))
    ids = np.argsort(x)
    xin = x[ids]
    yin = y[ids]
    
    for i in range (0,nbins):
        xmed[i] = np.mean(xin[i * ngals_per_bin: (i+1) * ngals_per_bin])
        sigma = np.std(yin[i * ngals_per_bin: (i+1) * ngals_per_bin])
        per = np.percentile(yin[i * ngals_per_bin: (i+1) * ngals_per_bin], [16, 50, 84])
        result[0, i] = per[1]
        result[1, i] = per[0]
        result[2, i] = per[2]
...
def wmedians(x=None, y=None, xbins=None, low_numbers=False, nmin=10):
    ...
    for i in range (0,nbins):
        ...
        if(len(x[ind]) >= nmin):
            ...
            obj_bin = len(x[ind])
            ybin    = y[ind]
            per = np.percentile(ybin, [16, 50, 84])
            result[0, i] = per[1]
            result[1, i] = per[0]
            result[2, i] = per[2]
...
def weighted_quantile(values, quantiles, sample_weight=None, 
    ...
    quantiles_values = np.zeros(shape = len(quantiles))
    for i,q in enumerate(quantiles):
        ind = np.where(abs(weighted_quantiles - q) == min(abs(weighted_quantiles - q)))
        quantiles_values[i] = values[ind][0]
    print("aca voy 3", quantiles_values)
    return quantiles_values #np.interp(quantiles, weighted_quantiles, values)
