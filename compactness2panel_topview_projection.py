...
fields = {'ExclusiveSphere/50kpc': (
            ...
            'LinearMassWeightedIronOverHydrogenOfStars', 'LinearMassWeightedMagnesiumOverHydrogenOfStars'
         )}
...
track_selected = np.asarray(track_id_in[mask_positive], dtype=np.int64)
exsitu_fracs = np.full(track_selected.shape, np.nan, dtype=np.float32)
...
if os.path.exists(h5path):
    with h5py.File(h5path, "r") as fh:
        if "stars" not in fh:
            ...
            # Read in chunks: columns 0 (track_id) and 3 (fraction)
            chunk = 500_000  # tune 2e5–1e6
            tids_all = np.empty(nrows, dtype=np.int64)
            fracs_all = np.empty(nrows, dtype=np.float32)

            for i0 in range(0, nrows, chunk):
                i1 = min(nrows, i0 + chunk)
                block = dset[i0:i1, :]          # reads only this slice
                tids_all[i0:i1] = block[:, 0].astype(np.int64, copy=False)
                fracs_all[i0:i1] = block[:, 3].astype(np.float32, copy=False)

            # Sort once, then fast vectorised lookup
            order = np.argsort(tids_all)
            tids_s = tids_all[order]
            fracs_s = fracs_all[order]

            pos = np.searchsorted(tids_s, track_selected)
            ok = (pos < tids_s.size) & (tids_s[pos] == track_selected)
            exsitu_fracs[ok] = fracs_s[pos[ok]]

            print(f"Loaded+matched ex-situ in vectorised mode: {ok.sum()} / {track_selected.size}")
else:
...
ax_proj.set_ylabel("Median [Mg/Fe] (dex)")
...
ax_proj.plot(cbin_centers, cmed, color="C0", lw=2)
ax_proj.fill_between(cbin_centers, clow, chigh, color="C0", alpha=0.25)
ax_proj.set_xlabel(r"Compactness (lg[$M_\odot$] - 1.5 lg[r/kpc])")
ax_proj.set_ylabel("Median exsitu mass fraction")
ax_proj.set_title("Projection: median & 16–84 pct")
ax_proj.grid(True)
...
ax_proj.fill_between(cbin_centers, clow, chigh, color="C0", alpha=0.25)
ax_proj.set_xlabel(r"Compactness (lg[$M_\odot$] - 1.5 lg[r/kpc])")
ax_proj.set_ylabel("Median lum-weighted age (Gyr)")
ax_proj.set_title("Projection: median & 16–84 pct")
ax_proj.grid(True)
...
ax_main.set_xlabel(r"Compactness (lg[$M_\odot$] - 1.5 lg[r/kpc])")
ax_main.set_ylabel("log10(sSFR / yr$^{-1}$)")
...
ax_proj.fill_between(cbin_centers, clow, chigh, color="C0", alpha=0.25)
ax_proj.set_xlabel(r"Compactness (lg[$M_\odot$] - 1.5 lg[r/kpc])")
ax_proj.set_ylabel("Median log10(sSFR / yr$^{-1}$)")
ax_proj.set_title("Projection: median & 16–84 pct")
ax_proj.grid(True)
